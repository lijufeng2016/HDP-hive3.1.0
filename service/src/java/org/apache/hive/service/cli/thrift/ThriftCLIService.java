...
public abstract class ThriftCLIService extends AbstractService implements TCLIService.Iface, Runnable {
     ...
     * @param mappings 规则，白名单黑名单都可以匹配
     * @param targetRule 期望的匹配结果
     * @return boolean
     */
    private boolean getMatchResult(String userName, String ipAddress, String mappings,boolean targetRule) {
        if (StringUtils.isNotBlank(mappings)) {
            String[] split = mappings.split(",");
            for (int i = 0; i < split.length; i++) {
                String mapping = split[i];
                String[] ipName = mapping.split(":");
                String ip = ipName[0];
                String names = ipName[1];
                String[] nameSit = names.split("\\|");
                String ipOrSeg = ip.replace(".*","");
                // 匹配网段
                if(ipAddress.contains(ipOrSeg)){
                    // 匹配到*代表所有用户
                    if(names.equals("*")){
                        return targetRule;
                    }
                    // 便利匹配用户
                    for (int j = 0; j < nameSit.length; j++) {
                        String name = nameSit[j];
